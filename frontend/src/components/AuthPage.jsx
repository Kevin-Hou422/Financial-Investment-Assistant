      ...
      <div className="relative z-10 w-full max-w-md space-y-8">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 shadow-2xl shadow-black/40 space-y-5">
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-gray-800 font-semibold rounded-xl px-4 py-3 transition-all duration-200 shadow-sm"
          >
            ...
            {['signin', 'signup'].map((m) => (
              <button key={m} type="button"
                ...
                className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all duration-200 ${
                  mode === m ? 'bg-violet-600 text-white shadow-md' : 'text-gray-400 hover:text-white'
                }`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-bold rounded-xl py-3 transition-all duration-200 shadow-lg shadow-violet-500/20 text-sm disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
              {loading && (
